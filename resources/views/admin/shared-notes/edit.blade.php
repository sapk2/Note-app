    ...
    <div class="container flex justify-center mb-2">
        <div class="max-w-md w-full bg-gray-600 p-4 rounded-lg shadow-lg">
            ...
            <form action="{{route('note.update',$notes->id)}}" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    <label for="is_viewed" class="flex items-center text-gray-500 dark:text-gray-400">
                        <input type="checkbox" name="is_viewed" id="is_viewed" value="1" class="mr-2" 
                            {{ old('is_viewed', $notes->is_viewed) ? 'checked' : '' }}> Viewed
                    </label>
                    @error('is_viewed')
                        <div class="text-red-500 text-sm">{{ $message }}</div>
                    @enderror
